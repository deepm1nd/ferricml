...
fn main() -> anyhow::Result<()> {
    ...
    let b = 1; let t = 8; let d = 32;
    let input_idx = vec![102, 51, 92, 14, 106, 71, 60, 20];
    let mut h_data = vec![0.0f32; b * t * d];
    let emb_binding = tok_embeddings.storage();
    let emb_data = match emb_binding.as_ref() { Storage::Cpu(s) => s.as_slice::<f32>() };
    for i in 0..t {
        let idx = input_idx[i];
        for j in 0..d { h_data[i * d + j] = emb_data[idx * d + j]; }
    }
    let h = Tensor::new(h_data, vec![b, t, d]);
    let norm_h = ffn_norm.forward(&h);
    let w1_out = w1.forward(&norm_h);
    let w2_out = w2.forward(&norm_h);
    let silu_w1 = silu(&w1_out);
    let swiglu = ferricml::cpu::ops::mul(&silu_w1, &w2_out);
    let ffn = w3.forward(&swiglu);
    ...
    let output = Tensor::new(final_data, vec![b, t, d]);

    let output_vec = match output.storage().as_ref() { Storage::Cpu(s) => s.as_slice::<f32>().to_vec() };
    let mut out_file = File::create("validation/tinyllama/ferric_output.json")?;
